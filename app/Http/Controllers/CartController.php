    ...
    public function addToCart(Request $request)
    {
        ...
        if(auth()->user()->booksInCart->contains($book)){
            ...
            if($newQuantity > $book->number_of_copies) {
                session()->flash('warning_message','لم تتم اضافة الكتاب,لقد تجاوزت عدد النسخ الموجودة لدينا ,اقصى عدد موجود بامكانك حجزه من هذا الكتاب هو') . ($book->number_of_copies - auth()->user()->booksInCart->where('book_id',$book->id->first()->pivot->number_of_copies) . 'كتاب');
